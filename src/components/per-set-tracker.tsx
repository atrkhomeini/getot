...
export function PerSetTracker({
  ...
  const updateSet = (setNumber: number, field: keyof SetData, value: number | boolean | SetType) => {
    setSetsData(prev => {
  ...
  // Check if set meets target (considering set type)
  const meetsTarget = (set: SetData): { weight: boolean; reps: boolean; isOk: boolean } => {
    const weightMet = set.actual_weight >= set.target_weight
    const repsMet = set.actual_reps >= set.target_reps
    
    // Warmup and Failure sets are always "OK" - don't show red warning
  ...
  const getPerformanceIndicator = (set: SetData) => {
    ...
    if (set.set_type === 'failure') {
      return { icon: Flame, color: 'text-red-500', label: 'Failure set' }
    }
    
    // Warmup set
    if (set.set_type === 'warmup') {
      return { icon: Sun, color: 'text-yellow-500', label: 'Warm-up' }
    }
    
    // Normal set
    if (isOk) {
      return { icon: TrendingUp, color: 'text-green-500', label: 'Target met' }
    }
  ...
  const calculateProgress = () => {
    ...
    // Only count normal and failure sets for progress
    const workingSets = setsData.filter(s => s.set_type !== 'warmup')
    if (workingSets.length === 0) return 100 // All warmup = 100%
    const completedSets = workingSets.filter(set => set.completed).length
    return (completedSets / workingSets.length) * 100
  }

  const getTotalReps = () => setsData.reduce((sum, set) => sum + set.actual_reps, 0)
  
  const getTotalVolume = () => {
    // Only count working sets (exclude warmup)
    return setsData
      .filter(s => s.set_type !== 'warmup')
      .reduce((sum, set) => sum + (set.actual_weight * set.actual_reps), 0)
  }
  
  const getCompletedSetsCount = () => setsData.filter(set => set.completed).length
  
  const getWorkingSetsCount = () => setsData.filter(s => s.set_type !== 'warmup').length
  
  const getAvgWeight = () => {
    // Only average working sets
    const workingSets = setsData.filter(s => s.set_type !== 'warmup' && s.completed)
    if (workingSets.length === 0) return 0
    return workingSets.reduce((sum, set) => sum + set.actual_weight, 0) / workingSets.length
  }
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-4 flex items-center justify-center gap-4 text-xs font-mono">
            <div className="flex items-center gap-1">
              <span className={cn("w-4 h-4 rounded", SET_TYPE_CONFIG.warmup.color)} />
              <span className="text-muted-foreground">W: {getSetTypeCount('warmup')}</span>
            </div>
            <div className="flex items-center gap-1">
              <span className={cn("w-4 h-4 rounded", SET_TYPE_CONFIG.normal.color)} />
              <span className="text-muted-foreground">N: {getSetTypeCount('normal')}</span>
            </div>
            <div className="flex items-center gap-1">
              <span className={cn("w-4 h-4 rounded", SET_TYPE_CONFIG.failure.color)} />
              <span className="text-muted-foreground">F: {getSetTypeCount('failure')}</span>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          return (
            <Card
              ...
              className={cn(
                ...
                !isOk && set.set_type === 'normal' && "border-red-300"
              )}
            >
                  ...
                  <div className="flex gap-2">
                    {(['warmup', 'normal', 'failure'] as SetType[]).map((type) => {
                      ...
                      return (
                        <button
                          key={type}
                          onClick={() => updateSet(set.set_number, 'set_type', type)}
                          className={cn(
                            "flex-1 py-2 px-3 rounded-lg font-mono font-bold text-sm transition-all border-2",
                            isActive 
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                    ...
                    <div className={cn(
                      ...
                      {set.set_type === 'warmup' ? (
                        <>
                          ...
                          <span>Warm-up (light weight)</span>
                        </>
                      ) : set.set_type === 'failure' ? (
                        <>
                          <Flame className="w-4 h-4" />
                          <span>Failure: {set.actual_reps} reps achieved</span>
                        </>
                      ) : (
                        <>
                          {weightMet ? <CheckCircle className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                    ...
                    <div className={cn(
                      ...
                      {set.set_type === 'warmup' ? (
                        <>
                          <Sun className="w-4 h-4" />
                          <span>Warm-up (low intensity)</span>
                        </>
                      ) : set.set_type === 'failure' ? (
                        <>
                          <Flame className="w-4 h-4" />
                          <span>Failed at {set.actual_reps} reps</span>
                        </>
                      ) : (
                        <>
                          {repsMet ? <CheckCircle className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
